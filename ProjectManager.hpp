...
//////////////////////////////////////////////////////////////////////////////////////////////
        ...
        void init() {
            ...
            const Movable playerKeys[4] = [{'z','q','s','d'}, {'i','j','k','l'}, {'t','f','g','h'}, {'o','l','p','m'} ];
            for (int i = 0; i < map.size(); i++) {
                for (int j = 0; j < map[i].size(); j++) {
                    if (map[i][j] != 'b') { continue; }
                    if (i > nbPlayer) { break; }
                    ...
                    _ecs.add_component<Drawable>(player, {LoadTexture("../assets/player" +  std::to_string(i) + ".png")});
                    _ecs.add_component<Movable>(player, playerKeys[i]);
